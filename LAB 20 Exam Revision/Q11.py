...
def get_sorted_list(search_tree):
    if not search_tree:
        return []
    # binary search tree descending order is right centre left
    right = get_sorted_list(search_tree.get_right())
    centre = [search_tree.get_data()]
    left = get_sorted_list(search_tree.get_left())
